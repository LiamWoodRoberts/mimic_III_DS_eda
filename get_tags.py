...
def load_sections(title):
    sections = run(title)
    sections = [i for i in sections if i != "NOT FOUND"]
    return sections
...
def load_entities(ent_type):
    ents = np.load(f"./{ent_type}_entities.npy")
    ents = [d.lower() for d in ents if d.lower()]
...
def create_ent_df():
    ...
    med_ents = pd.DataFrame()

    for ent_type in ['dose','unit','route']:
        ents = pd.DataFrame()
        ents["Name"] = load_entities(ent_type)
        ents["Entity"] = ent_type
        med_ents = pd.concat([med_ents,ents],axis=0)
    ...
    df = pd.concat([drugs,med_ents,other_ents],axis=0)
